 ...
 * @author Ruba
 */
class TestVersion1Backup {
    static String reviews, reviewFile,annFile;
    ...
    reviews = new Scanner(new File(reviewFile)).useDelimiter("\\Z").next();
    ...
    while (annData.hasNext()) {
            ...
            reviews = reviews.substring(0,startIndex+diff) + tagAndIndex[0] +"/"+ reviews.substring(startIndex+diff,endIndex + diff).toLowerCase().replaceAll(" ", "_")+reviews.substring(endIndex+diff,reviews.length());
            diff+= tagAndIndex[0].length() + 1 ;
            }
            else{
            reviews = reviews.substring(0,startIndex+diff) + tagAndIndex[0] +reviews.substring(endIndex+diff,reviews.length());
            diff+= tagAndIndex[0].length() - reviews.substring(startIndex+diff,endIndex + diff).length();
            }
        }

    formattedSentences = Arrays.asList(reviews.split("\n\n"));
    ...
    while(fullstop>=0){
        ...
        System.out.println("No of reviews : "+formattedSentences.size());*/
    }
...
for(String review : formattedSentences) {
            ...
            if(candidateAspect.size() >= 1){
                ...
                for(int l=0; l<words.length; l++){
                    ...
                    while(temp.indexOf(wordN)>=0){
                        ...
                        if(l != n)
                        availability[position]+= enitityFactor*(1/Math.abs(l - n));
                        else
                ...
                boolean success = checkParent(words,n,winningAspect);
                    if(!success){
                        //if(anAspect)
